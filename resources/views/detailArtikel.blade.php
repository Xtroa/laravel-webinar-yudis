...
<body>
    <header>
        ...
        <img class="headImage"src='{{ asset('storage/' .$data->thumbnail) }}' style="width: 100%; height: 480px;"></img>
        ...
        <div class="headTitle">
            ...
            <div style="color: white;">
                <h1>{{$data->title}}</h1>
                <h3 class="subheading" style="margin-top: 10px;">" {{$data->excerpt}} "</h3>
    ...
    <div class="container main" style="margin-top: 50px; margin-bottom: 50px;">
        <h6 class="" style="align-center; margin-bottom: 50px;">
            <em>Diposting tanggal : {{$data->created_at->format('d F Y ')}} oleh {{$data->user->name}} </em>
        </h6>
        <div style="text-align: center; margin-bottom: 50px;">
        <img src="{{ asset('storage/' .$data->thumbnail) }}" width="500px" alt=""></td>
        </div>
        {!! $data->content !!}
    </div>
    @endforeach
